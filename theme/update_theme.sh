...
_update() {
    local clone_pid copy_pid
    _clean_up

    cd "$SCRIPT_DIR" || _exit_script

    git clone -q "git@github.com:${GITHUB_REPO_OWNER}/${GITHUB_REPO_NAME}.git" &
    clone_pid=$!
    _spinner $clone_pid "Downloading fresh copy of theme" "Downloaded fresh copy of theme"

    if [[ ! -d "${SCRIPT_DIR:?}/${GITHUB_REPO_NAME:?}/" ]]; then
        echo "Failed to download fresh copy of theme from github.com"
        _exit_script
    fi

    cp -R --update "${SCRIPT_DIR:?}/${GITHUB_REPO_NAME:?}/themes/${THEME_FOLDER:?}" "${DESTINATION_DIRECTORY:?}/" &
    copy_pid=$!

    _spinner $copy_pid "Updating theme directory" "Updated theme directory"
...
_clean_up() {
    if [ -d "${SCRIPT_DIR:?}/${GITHUB_REPO_NAME:?}/" ]; then
        rm -rf "${SCRIPT_DIR:?}/${GITHUB_REPO_NAME:?}/"
